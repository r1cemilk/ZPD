...
function animate(time) {
  for (let i = 0; i < traffic.length; i++) {
    traffic[i].update(road.borders, []); // empty array cause if i'd add traffic there, the traffic would get damaged as well (the user is also considered traffic and would damage itself, so should write codde preventing that if i were to use traffic instead of empty array)
  }
  for (let i = 0; i < cars.length; i++) {
